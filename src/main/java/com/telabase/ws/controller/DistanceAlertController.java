	...
	@RequestMapping(value = "/api/distancealert/list", method = RequestMethod.GET)
		...
		@RequestMapping(value = "/api/distancealert/status/toggle", method = RequestMethod.POST)
		@ResponseBody
		public WSResponse toggleStatus(HttpServletRequest request) {

			DistanceAlertDAO serve = new DistanceAlertDAO();
			DistanceAlert d = serve.findById(Long.parseLong(request.getParameter("id")));
			...
			try {
				...
				e.printStackTrace();
				return new WSResponse(""+d.getId(), WSResponse.STATUS_FAIL);
			}
			return new WSResponse(""+d.getId(), WSResponse.STATUS_SUCCESS);
		}
